...
out <- "07_03_CleanHuman_CleanSpikeIns/"
...
sample.annot <- fread("metadata/Aggregate_Human1_2.csv")
...
# PLOTS: COMPARE ESTIMATED TO SPIKE IN TRUTH -------------------------------------------
...
for(dataType in c("original", "soupX", "spikeIns")){
  for(orgXX in c("human", "mouse")){ # Organism of spike-ins
    datX <- data.tpm.spike; 
