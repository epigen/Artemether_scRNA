...
out <- "30_05_ClusteringExperiments/"
...
for(typex in c("raw", "corrected")){
  ...
  pbmc.data <- pbmc.data[genes.to.use,meta$rn]
  pbmcOLD <- CreateSeuratObject(raw.data = pbmc.data, min.cells = 0, min.genes = 0, scale.factor=SCALE.FACTOR, project = "X")
